(ns s-exp.k7-bench-long
  "Long-running benchmark suite for k7 — uses full criterium sample counts
   (~60 samples) for statistically robust results. Takes significantly longer
   than k7-bench but produces tighter confidence intervals.

   Run with: clojure -M:bench-run-long

   Benchmarks:
     1. enqueue! throughput — raw write rate per fsync strategy (incl. :sync)
  ...
  (let [payload (.getBytes "hello-world-benchmark-payload-32b!")
    ...
    (doseq [strategy [:flush :async :sync]]
      ...
      (with-tmp-queue [q {:fsync-strategy strategy}]
        (let [result (crit/benchmark (enqueue-n! q payload n) {:os true :runtime true :verbose true})]
    ...
    (doseq [batch-size [1 16 64 256 1024]]
      ...
      (with-tmp-queue+cg [q cg {:fsync-strategy :flush} {:cursor-fsync-strategy :async}]
        ...
        (let [result (crit/benchmark
                      (do (k7/seek! cg 0)
                          (drain-all! cg n))
                      {:os true :runtime true :verbose true})]
          (crit/report-result result)
          (print-msgs-per-sec result n))))))
...
(defn bench-segment-roll []
  (let [payload-size  1024
    ...
    (with-tmp-queue [q {:fsync-strategy :flush :segment-size (* 256 1024 1024)}]
      (let [result (crit/benchmark (k7/enqueue! q payload) {:os true :runtime true :verbose true})]
        (crit/report-result result)
        (print-msgs-per-sec result 1)))
    ...
    (let [frame-size   (s-exp.k7/aligned-frame-size payload-size)
      ...
      (let [sorted (doto (long-array times) java.util.Arrays/sort)
            ...
            p95    (aget sorted (int (* 0.95 trials)))
            p99    (aget sorted (int (* 0.99 trials)))]
        (println (format "  mean=%.1fµs  p50=%.1fµs  p95=%.1fµs  p99=%.1fµs"
                         (/ mean 1000.0) (/ p50 1000.0) (/ p95 1000.0) (/ p99 1000.0)))))))
...
(defn -main [& args]
  (let [selected (set args)
        benches  (if (seq selected)
                   ...
                   all-benches)]
    (println "╔══════════════════════════════════════════════╗")
    (println "║        k7 benchmark suite (long run)         ║")
    (println "╚══════════════════════════════════════════════╝")
